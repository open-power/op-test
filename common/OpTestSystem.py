...
class OpTestSystem(object):
    ...
    def __init__(self,
        ...
        # system console state tracking, reset on boot and state changes, set
        # when valid
        self.PS1_set = -1
        self.SUDO_set = -1
        self.LOGIN_set = -1
        ...
        # string to define petitboot kernel cat /proc/version column 3, change
        # if using debug petitboot kernel
        self.openpower = 'openpower'

        # dictionary used in sorted order
        # column 1 is the string, column 2 is the action
        # normally None is the action, otherwise a handler mostly used for
        # exceptions
        ...
        # to basically turn off reconnect based on stale buffers set threshold
        # equal to watermark, e.g. 100
        if isinstance(self.console, OpTestIPMI.IPMIConsole):
            self.threshold_petitboot = 12  # stale buffer check
            # long enough to skip the refresh until kexec, stale buffers need
            # to be jumped over
            ...
            # less reliable connections, ipmi act/deact does not trigger
            # default boot cancel
    ...
    def login_callback(self, **kwargs):
        ...
        # raise the WaitForIt exception to be bubbled back to recycle early
        # rather than having to wait the full loop_max
    ...
    def guard_callback(self, **kwargs):
        ...
        guard_exception = UnexpectedCase(
            state=self.state, msg="We hit the guard_callback value={}, manually restart the system".format(kwargs['value']))
        self.state = OpSystemState.UNKNOWN_BAD
        self.stop = 1
    ...
    def skiboot_callback(self, **kwargs):
        ...
        skiboot_exception = UnexpectedCase(
            state=self.state, msg="We hit the skiboot_callback value={}, manually restart the system".format(kwargs['value']))
        self.state = OpSystemState.UNKNOWN_BAD
        self.stop = 1
    ...
    def goto_state(self, state):
        ...
        # if user overrides from command line and machine not at desired state
        ...
        while 1:
            ...
            # transition from states invalidate the previous PS1 setting, so
            # clear it
    ...
    def check_kernel(self):
        ...
        if rc in [0, 1, 5, 6]:
            ...
            return OpSystemState.UNKNOWN
        sys_pty.sendline(
            "cat /proc/version | cut -d ' ' -f 3 | grep %s; echo $?" % (self.openpower))
        time.sleep(0.2)
        rc = sys_pty.expect(
    ...
    def wait_for_it(self, **kwargs):
        ...
        if kwargs['fresh_start']:
            # new connect gets new pexpect buffer, stale buffer from power off
            # can linger
        ...
        while (x <= kwargs['loop_max']):
            ...
            # if we found a hit on the callers string return it, otherwise keep
            # looking
            if working_r != -1:
                ...
                log.debug("\n *** WaitForIt CURRENT STATE \"{:02}\" TARGET STATE \"{:02}\"\n"
                          ...
                          " *** WaitForIt Refresh=\"{}\" Buffer Kicker=\"{}\" - Kill Cord=\"{:02}\"\n".format(self.state, self.target_state,
                                                                                                              x, reconnect_count, kwargs['loop_max'], kwargs[
                                                                                                                  'timeout'], kwargs['threshold'],
                ...
                if kwargs['last_try']:
                    ...
                    try:
                        ...
                        if last_try_r != -1:
                            ...
                            raise WaitForIt(expect_dict=kwargs[
                                            'expect_dict'], reconnect_count=reconnect_count)
                    except Exception as e:
                        raise e
                raise WaitForIt(expect_dict=kwargs[
                                'expect_dict'], reconnect_count=reconnect_count)
    ...
    def check_it(self, **kwargs):
        ...
        if (check_r + 1) in range(len(check_base_seq) + 1, len(check_expect_seq) + 1):
            ...
            if check_expect_dict[check_expect_seq[check_r]]:
                try:
                    # this calls the handler callback, mostly intended for
                    # raising exceptions
                    ...
                    if self.ignore == 1:  # future use, set this flag in a handler callback
                        ...
                        # this allows special handling without interrupting the
                        # waiting for a good case
                        return -1
                except Exception as e:
                    # if a callback handler raised an exception this will catch
    ...
    def run_REBOOT(self, target_state):
        ...
        # if run_REBOOT is used in the future outside of first time need to
        # review previous_state handling
        sys_pty = self.console.get_console()
    ...
    def run_IPLing(self, state):
        ...
        try:
            ...
            # see the tunables ipl_watermark and ipl_timeout for customization
            # for extra long boot cycles for debugging, etc
    ...
    def run_PETITBOOT(self, state):
        ...
        raise UnknownStateTransition(
            state=self.state, msg="OpTestSystem in run_PETITBOOT and something caused the system to go to UNKNOWN")
    ...
    def run_BOOTING(self, state):
        ...
        try:
            ...
            # see the tunables booting_watermark and booting_timeout for
            # customization for extra long boot cycles for debugging, etc
    ...
    def get_petitboot_prompt(self):
        ...
        if pes_rc in [0, 1]:
            ...
            # unblock in case connections are lost during state=4 the
            # get_console/connect can properly setup again
            self.block_setup_term = 0
        ...
        try:
            ...
            if not my_ip:
                if len(ip_lst) == 1:
                    ...
                    log.error(
                        "hostname -i does not provide valid IP, correct and proceed with installation")
        return my_ip
...
class OpTestOpenBMCSystem(OpTestSystem):
    ...
    def sys_power_soft(self):
        # self.rest.power_soft() currently rest command for softPowerOff
        # failing
        self.rest.power_off()
