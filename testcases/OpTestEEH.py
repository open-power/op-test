...
class OpTestEEH():
    ...
    def run_pe_4(self, addr, e, f, phb, pe, con):
        ...
        interface_count_old = self.get_count_of_interdaces_in_pci_devices(pe)
        ...
        interface_count = self.get_count_of_interdaces_in_pci_devices(pe)
    ...
    #
    def get_count_of_interdaces_in_pci_devices(self, pci_addr):
        cmd = "ls -l /sys/class/*/ -1"
        count = 0
        for output in self.cv_IPMI.run_host_cmd_on_ipmi_console(cmd):
            if pci_addr in output:
                count += 1
        return count
