...
is visible in three stages:
  ...
  2. Verify the added vNIC MAC addresses are present in the LPAR profile
     via HMC (SMS menu layer confirmation).
...
or passed on the command line:
  ...
  ``vnic_phys_port_ids``    - Space-separated physical port numbers on the SRIOV
...
space-separated list of zero-based port indexes (e.g. ``0 1 2``).

Usage::

    op-test --config-file vnic_offline_add.conf \\
            --run testcases.OpTestVnicOfflineLpar.OpTestVnicOfflineLpar
'''
...
# ---------------------------------------------------------------------------
BOOT_SETTLE_TIME = 30           # seconds to settle before probing after state change
...
class OpTestVnicOfflineLpar(unittest.TestCase):
    '''
    Test: Add vNICs (one per supplied physical port ID) while the LPAR
    is offline, verify all vNICs via HMC SMS menu profile check, verify all
    vNICs in the host OS, then remove them and verify a clean boot.

    Steps
    -----
    1. Ensure the LPAR is powered off (Not Activated).
    2. Add vNICs to the LPAR profile via ``chsyscfg -r prof`` while offline.
       Confirm each entry via ``lssyscfg``.
    3. Confirm vNIC slot and MAC presence in the LPAR profile via HMC
       ``lssyscfg`` (SMS menu layer verification).
    ...
    def setUp(self):
        ...
        # Space-separated list of physical port IDs; argparse gives a string
        # with default "0 1 2"; split here into a list of ints.
        self.vnic_phys_port_ids = [
            int(p) for p in args.vnic_phys_port_ids.split()]
        ...
        if len(self.vnic_phys_port_ids) < 1:
            raise OpTestError(
                "vnic_phys_port_ids must list at least one port ID "
    ...
    def _verify_vnics_in_sms_menu(self, prof, slots):
        '''
        Confirm every vNIC slot in ``slots`` is present in the LPAR profile
        via HMC ``lssyscfg``.  On PowerVM, the SMS menu is the pre-OS firmware
        environment managed by the HMC; MAC address assignment and adapter
        enumeration at this layer is confirmed directly through the profile
        data rather than through a bootloader shell.

        :param prof:  string, LPAR profile name to query
        :param slots: list of ints, virtual slot numbers to verify
        :returns: dict mapping slot (int) -> mac_address (str) for all slots
        :raises: OpTestError if any slot is missing from the profile
        '''
        log.info("=== SMS menu check: verifying %d vNIC slot(s) %s in "
                 "profile '%s' via HMC lssyscfg ===", len(slots), slots, prof)
        entries = self._get_profile_vnic_entries(prof)
        slot_to_mac = {}
        for entry in entries:
            ...
            for prop in parts:
                if prop.startswith('mac_addr='):
                    mac = prop.split('=', 1)[1].strip()
                    break
            slot_to_mac[entry_slot] = mac

        missing = []
        confirmed = {}
        for slot in slots:
            if slot in slot_to_mac:
                mac = slot_to_mac[slot]
                log.info("  SMS menu CONFIRMED: slot=%-4d  MAC=%s", slot, mac)
                confirmed[slot] = mac
            else:
                log.error("  SMS menu MISSING:   slot=%-4d  not in profile '%s'",
                          slot, prof)
                missing.append(slot)

        if missing:
            raise OpTestError(
                "The following vNIC slot(s) were NOT found in profile '%s' "
                "during SMS menu check: %s" % (prof, missing))
        return confirmed
    ...
    def _ensure_lpar_offline(self):
        ...
        | (SMS menu)        | Wait for Not Activated                           |
        +-------------------+--------------------------------------------------+
        ...
        if state == OpHmcState.SHUTTING:
            ...
            # -- Running or Open Firmware (SMS menu) - issue immediate shutdown
    ...
    def runTest(self):
        # One virtual slot per physical port: slot 20 -> port[0], slot 21 -> port[1]
        slots = [20, 21]
        ...
        # ----------------------------------------------------------------
        # Step 2 - Add vNICs while LPAR is offline via chsyscfg -r prof
        # ----------------------------------------------------------------
        actual_prof = self._get_lpar_profile_name()
        vios_id = self._get_vios_id()
        adapter_id = self._get_sriov_adapter_id()

        log.info("=== Step 2: Adding %d vNIC(s) to profile '%s' while offline "
                 "(VIOS=%s, adapter_id=%s, slots=%s) ===",
                 len(slots), actual_prof, self.vios_name, adapter_id, slots)
        ...
        mac_addresses = []
        for slot, phys_port_id in zip(slots, self.vnic_phys_port_ids):
            ...
            if mac:
                log.info("  Added vNIC: slot=%-4d  port=%d  MAC=%s (profile)",
                         slot, phys_port_id, mac)
            else:
                log.info("  Added vNIC: slot=%-4d  port=%d  MAC=(pending - assigned at boot)",
                         slot, phys_port_id)
        ...
        # ----------------------------------------------------------------
        # Step 3 - SMS menu check: confirm vNIC slots in profile via HMC
        # ----------------------------------------------------------------
        log.info("=== Step 3: SMS menu MAC verification via HMC lssyscfg ===")
        sms_confirmed = self._verify_vnics_in_sms_menu(actual_prof, slots)
        log.info("----------------------------------------------------------")
        log.info("Step 3 PASSED - SMS menu vNIC slots confirmed via HMC:")
        for slot, mac in sms_confirmed.items():
            log.info("  Slot %-4d -> MAC %s  [HMC profile == SMS menu]", slot, mac)
        log.info("----------------------------------------------------------")
...
def suite():
    ...
    s.addTest(OpTestVnicOfflineLpar('runTest'))
    return s
