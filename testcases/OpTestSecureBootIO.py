...
class OpTestSecureBootIO(unittest.TestCase):
    ...
    def runTest(self):
        '''
        Orchestrates the 8-phase Secure Boot IO + HTX validation flow.
        ...
        Phase 7: Install HTX and run mdt.hd on block devices (opt-in).
        Phase 8: Check htxerr; shutdown HTX cleanly.

        Phases 7-8 are only entered when a subclass has assigned a
        configured OpTestHTXUtil to self._htx_util.  Non-block IO
        subclasses leave self._htx_util as None and skip these phases.
        '''
    ...
    def _htx_block_devices(self):
        '''
        Return the list of bare block device names to activate in HTX.
        Base implementation returns []; block-device subclasses override.
        '''
        return []
