...
from Exceptions import HostLocker, AES, ParameterCheck, HTTPCheck, UnexpectedCase, SSHSessionDisconnected
...
def response_json(response):
    ...
    try:
        ...
        raise HTTPCheck(
            msg="Unexpected problem getting JSON, Exception={}".format(e))
...
class OpTestUtil():
    ...
    def setup(self, config='HostLocker'):
        # we need this called AFTER the proper configuration values have been
        # seeded
        if config == 'AES':
        ...
        if self.conf.args.aes is not None:
            ...
            if lock:
                ...
                if envs is not None and len(envs) > 0:
                    if len(envs) <= 1:
                        for env in envs:
                            # working_id should NOT be kept to release upon
                            # exit
        ...
        if self.conf.args.hostlocker is not None:
            if self.conf.args.hostlocker_keep_lock is False:
                ...
                if rc == 0:
                    ...
                    # the info message may say released, due to exceptions
                    # thrown
    ...
    def build_proxy(self, proxy, no_proxy_ips):
        ...
        for ip in no_proxy_ips:
            ...
            try:
                ...
                raise HostLocker(
                    msg="Could not run 'ip' to check for no proxy?")
    ...
    def aes_release_reservation(self, res_id=None, env=None):
        ...
        try:
            r = self.conf.util_server.get(
                '/release-reservation.php', res_payload)
    ...
    def aes_lock_env(self, env=None):
        ...
        if env.get('state') == 'A':
            ...
            r = self.conf.util_server.get(
                '/enqueue-reservation.php', res_payload)
            if r.status_code != 200:
    ...
    def hostlocker_lock(self, args):
        ...
        try:
            r = self.conf.util_server.get(
                'host/%s/' % self.conf.args.hostlocker)
        except Exception as e:
        ...
        for key in args_dict.keys():
            for i in range(len(hostlocker_comment)):
                if key + ':' in hostlocker_comment[i]:
                    args_dict[key] = re.sub(
                        key + ':', "", hostlocker_comment[i]).strip()
                    break
    ...
    def try_recover(self, term_obj, counter=3):
        # callers beware that the connect can affect previous states and
        # objects
    ...
    def try_sendcontrol(self, term_obj, command, counter=3):
        ...
        if rc != 0:
            ...
            # if we get back here we still fail but have a working prompt to
            # give back
    ...
    def set_PS1(self, term_obj, pty, prompt):
        ...
        # order of execution and commands are sensitive here to provide
        # reliability
        ...
        # pause for first time setup, buffers you know, more sensitive in
        # petitboot shell, pexpect or console buffer not sure
        time.sleep(0.2)
        rc = pty.expect(
        ...
        if rc == 0:
            ...
            term_obj.close()
            try:
                # special case to allow calls back to connect which is where we
                # probably came from
                ...
                # if try_recover bails we leave things blocked, they'll get
                # reset
                self.try_recover(term_obj, counter=3)
                # if we get back here we have a new prompt and unknown console
                # in future if state can change or block flags can change this
                # needs revisted
                ...
                time.sleep(0.2)
                pty.sendline()  # needed to sync buffers later on
                # pause for first time setup, buffers you know, more sensitive
                # in petitboot shell, pexpect or console buffer not sure
                time.sleep(0.2)
                rc = pty.expect(
                    [expect_prompt, pexpect.TIMEOUT, pexpect.EOF], timeout=10)
                if rc == 0:
    ...
    def get_sudo(self, host, term_obj, pty, prompt):
        ...
        # we need to cleanly ignore everything but password and then we blindly
        # next do PS1 setup, ignoring who knows what
        if rc == 0:
            pty.sendline(my_pwd)
    ...
    def setup_term(self, system, pty, ssh_obj=None, block=0):
        ...
        # CANNOT CALL GET_CONSOLE OR CONNECT from here since get_console and
        # connect call into setup_term
        if block == 1:
            return
        ...
        if rc in [1, 2, 3]:
            ...
            # ssh port 22 can get in which uses sshpass or Petitboot, do this
            # after set_PS1 to make sure we have something
        ...
        if rc == 4:
            ...
            if term_obj.setup_term_quiet == 0:
                ...
                # this case happens when detect_target sets the quiet flag and
                # we are timing out
                log.info(
    ...
    def handle_password(self, term_obj, pty, command):
        ...
        if rc == 0:
            # with unknown prompts and unknown environment unable to capture
            # echo $?
            echo_rc = 0
    ...
    def try_command(self, term_obj, command, timeout=60):
        ...
        expect_prompt = self.build_prompt(term_obj.prompt) + "$"
        # if previous caller environment leaves buffer hung can show up here,
        # e.g. PS2 prompt
        pty = term_obj.get_console()
        ...
        try:
            ...
            pass  # nothing there
        # if we are running 'sudo -s' as root then catch on generic # prompt,
        # restore env
...
class Server(object):
    ...
    def get(self, url, params={}):
        try:
            r = self.session.get(self._url(url), params=params,
    ...
    def post(self, url, params):
        try:
            ...
            raise HostLocker(
                msg="Requests post problem, Exception={}".format(e))
    ...
    def delete(self, url):
        try:
            ...
            raise HostLocker(
                msg="Requests delete problem, Exception={}".format(e))
    ...
    def get_json(self, url, accept_empty=False, params={}):
        ...
        if response.status_code != 200:
            raise HTTPCheck(msg="Requests get_json problem, unexpected error '{}'".format(
