...
config_file = "/root/op-test/CONFIG.YAML"
...
class OpTestSysinfo():
    ...
    def __init__(self):
        with open(config_file, "r") as file:
            ...
            list_of_hmc_commands = self.config_actions["LINUX"]["HMC-COMMANDS"]
    ...
    def get_HMCconfig(self, pty, prompt):
        ...
        ################ HMC INFO ####################
        #Details of HMC software version, release, service pack, and build level
        list_of_hmc_commands = self.config_actions["LINUX"]["HMC-COMMANDS"]
        print("Getting HMC software version, release, service pack, and build level")
        allsysinfo=[]
        for index, each_cmd in enumerate(list_of_hmc_commands, start=0):
             if re.search('SYS', each_cmd):
                 for ele in allsysinfo:
                     new_cmd=re.sub('SYS',ele,each_cmd)
                     try:
                ...
                if re.search('list', str(type(output)) ):
                    allsysinfo = list(output)
                    print('ALL SYSINFO NAMES',allsysinfo)
