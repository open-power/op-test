...
class Petitbooti18n(unittest.TestCase):
    ...
    def runTest(self):
        ...
        rc = raw_pty.expect(['Petitboot', pexpect.TIMEOUT, pexpect.EOF], timeout=5)

        raw_pty.send("l") # key press L
        rc = raw_pty.expect(['Deutsch', pexpect.TIMEOUT, pexpect.EOF], timeout=5)
        rc = raw_pty.expect(['English', pexpect.TIMEOUT, pexpect.EOF], timeout=5)
        rc = raw_pty.expect([u'Espa\u00f1ol'.encode('utf-8'), pexpect.TIMEOUT, pexpect.EOF], timeout=5)
        rc = raw_pty.expect([u'Fran\u00e7ais'.encode('utf-8'), pexpect.TIMEOUT, pexpect.EOF], timeout=5)
        rc = raw_pty.expect(['Italiano', pexpect.TIMEOUT, pexpect.EOF], timeout=5)
        rc = raw_pty.expect([u'\u65e5\u672c\u8a9e'.encode('utf-8'), pexpect.TIMEOUT, pexpect.EOF], timeout=5)
        rc = raw_pty.expect([u'\ud55c\uad6d\uc5b4'.encode('utf-8'), pexpect.TIMEOUT, pexpect.EOF], timeout=5)
        rc = raw_pty.expect([u'Portugu\u00eas/Brasil'.encode('utf-8'), pexpect.TIMEOUT, pexpect.EOF], timeout=5)
        rc = raw_pty.expect([u'\u0420\u0443\u0441\u0441\u043a\u0438\u0439'.encode('utf-8'), pexpect.TIMEOUT, pexpect.EOF], timeout=5)
        rc = raw_pty.expect([u'\u7b80\u4f53\u4e2d\u6587'.encode('utf-8'), pexpect.TIMEOUT, pexpect.EOF], timeout=5)
        rc = raw_pty.expect([u'\u7e41\u9ad4\u4e2d\u6587'.encode('utf-8'), pexpect.TIMEOUT, pexpect.EOF], timeout=5)
        ...
        rc = raw_pty.expect(['x=exit', pexpect.TIMEOUT, pexpect.EOF], timeout=10)
