        ...
        while not done:
            try:
                ...
                rc = self.c.pty.expect(
                    ["ISTEP 10. 1", "kdump: saving vmcore complete", "saved vmcore", "Elapsed time since"], timeout=600)
            except KernelFADUMP:
...
class KernelCrash_KdumpSMT(PowerNVDump):
    ...
    def runTest(self):
        ...
        for i in ["off", "2", "4"]:
            ...
            print("=============== Testing kdump with smt=%s ===============" % i)
            boot_type = self.kernel_crash()
            self.verify_dump_file(boot_type)
        self.setup_test()
        ...
        if self.is_lpar:
            for i in ["off", "2", "4"]:
                ...
                self.c.run_command("ppc64_cpu --smt=%s" % i)
                self.c.run_command("ppc64_cpu --smt")
                print("=============== Testing kdump with smt=%s and dumprestart from HMC ===============" % i)
                boot_type = self.kernel_crash(crash_type="hmc")
                self.verify_dump_file(boot_type)
