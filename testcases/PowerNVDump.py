...
class PowerNVDump(unittest.TestCase):
    ...
    def setUp(self):
        ...
        res = self.c.run_command_ignore_fail("cat /etc/os-release")
        if "Ubuntu" in res[0] or "Ubuntu" in res[1]:
            self.distro = "ubuntu"
        elif 'Red Hat' in res[0] or 'Red Hat' in res[1]:
            self.distro = 'rhel'
        else:
            raise self.skipTest("Test currently supported only on ubuntu and rhel")
...
class KernelCrash_KdumpNetwork(PowerNVDump):
    ...
    def setup_nfs(self):
        self.cv_SYSTEM.goto_state(OpSystemState.OS)
        if self.distro == "rhel":
            self.c.run_command("sed -e '/ssh/ s/^#*/#/' -i /etc/kdump.conf;")
            self.c.run_command("yum -y install nfs-utils")
    ...
    def runTest(self):
        pwd_less = self.c.run_command_ignore_fail("ssh -o \"StrictHostKeyChecking no\" -o  \"NumberOfPasswordPrompts=0\" %s \"echo\"" % self.server_ip)
...
class KernelCrash_KdumpSMT(PowerNVDump):
    ...
    def runTest(self):
        ...
        self.verify_dump_file(boot_type)
        self.setup_test()
        self.c.run_command("ppc64_cpu --smt=2")
        self.c.run_command("ppc64_cpu --smt")
        print("=============== Testing kdump with smt=2 ===============")
        boot_type = self.kernel_crash()
        self.verify_dump_file(boot_type)
        self.setup_test()
        self.c.run_command("ppc64_cpu --smt=4")
        self.c.run_command("ppc64_cpu --smt")
        ...
        print("=============== Testing kdump with single core ===============")
        boot_type = self.kernel_crash()
        self.verify_dump_file(boot_type)
        self.setup_test()
        self.c.run_command("ppc64_cpu --cores-on=1", timeout=600)
        self.c.run_command("ppc64_cpu --smt=off")
        print("=============== Testing kdump with single cpu ===============")
        boot_type = self.kernel_crash()
        self.verify_dump_file(boot_type)
