...
Example invocation:
  ./op-test --bmc-type FSP_PHYP \
            ...
            --run testcases.OpTestSMSNetworkAdapter.SMSNetworkAdapterDetection
'''
...
class SMSNetworkAdapterDetection(unittest.TestCase):
    '''
    ...
    def setUp(self):
        ...
        if self.bmc_type not in ('FSP_PHYP', 'EBMC_PHYP'):
            self.skipTest(
                "SMSNetworkAdapterDetection requires an HMC-managed LPAR "
    ...
    def _navigate_to_network_ports(self, pty):
        ...
        log.info("Selecting SMS option 2: Setup Remote IPL")
        ...
        log.info("Adapter listing not found on Remote IPL screen; "
                 "returning to main menu")
        self._navigate_to_main_menu(pty)

        # --- Main Menu -> 3: I/O Device Information ---
        log.info("Selecting SMS option 3: I/O Device Information")
        self._sms_send(
            pty,
            key='3',
        ...
        log.info("Selecting I/O Device Information sub-menu option 6: "
                 "Network ports")
        self._sms_send(
            pty,
            key='6',
    ...
    def _reboot_to_os_and_match_macs(self):
        ...
        log.info("Rebooting LPAR '%s' into the OS", self.lpar_name)
        self.cv_HMC.restart_lpar()
...
def suite():
    ...
    s.addTest(SMSNetworkAdapterDetection())
    return s
